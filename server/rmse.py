...
class TrainAndTest:
    ...
    def writeTrainData(self, path, sheet, train_sheet):
        ...
        with pd.ExcelWriter(path, engine="openpyxl", mode="a", if_sheet_exists='replace') as writer:
            self.shuffleData(train_subset_data).to_excel(writer, sheet_name=train_sheet, index=False)

    def writeTestData(self, path, sheet, test_sheet):
        data = pd.read_excel(path, sheet_name=sheet)
        ...
        with pd.ExcelWriter(path, engine="openpyxl", mode="a", if_sheet_exists='replace') as writer:
            self.shuffleData(test_subset_data).to_excel(writer, sheet_name=test_sheet, index=False)
...
def test10x(path, ref_sheet, train_sheet, test_sheet, dep, indep):
    writer.writeTrainData(path, ref_sheet, train_sheet)
    coeff = writer.getCoefficients(path, train_sheet, dep, indep)
    print(coeff)
    for i in range(1,11): 
        currTest = f"{test_sheet}_{i}"
        writer.writeTestData(path, ref_sheet, currTest)

        writer.writePredictions(path, currTest, coeff)
